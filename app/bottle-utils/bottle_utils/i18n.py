...
class I18NPlugin(object):
    ...
    def apply(self, callback, route):
        ...
        def wrapper(*args, **kwargs):
            original_path = request.environ.get('ORIGINAL_PATH') or request.fullpath  # Use 'or' for concise fallback

            query_string = request.environ.get('QUERY_STRING')
            if query_string:
                request.original_path = f"{original_path}?{query_string}"  # Use f-string for cleaner formatting
            else:
                request.original_path = original_path # No change needed if no query string
